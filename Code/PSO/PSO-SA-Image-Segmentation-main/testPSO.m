...
%MainOrg=imread('05.png');
%mainorg_tmp=MainOrg   %%%%%%
MainOrg=imread('D:/Code/Python_Code/Seg/k_means_result/test_new.bmp');%%%%%%
%MainOrg = ind2rgb(MainOrg,jet(256)); %# 将灰度图转换为彩色图  %%%%%%
...
% SA solver + PSO body
%SA_opts =
%optimoptions('simulannealbnd','display','iter','MaxTime',Itr,'PlotFcn',@pswplotbestf);
SA_opts = optimoptions('particleswarm','display','iter','MaxTime',Itr,'PlotFcn',@pswplotbestf);
...
%
medgray = medfilt2(SA_Segmented,[2 2]);
%
...
csvwrite('D:/Code/Project/PSO_image_segmentation/PSO-SA-Image-Segmentation-main/SA_Segmented.csv',SA_Segmented)
%title(['PSO Gray Segmented, Clusters = ' num2str(clusteres)]);
figure();
imshow(medgray,[]);
%
